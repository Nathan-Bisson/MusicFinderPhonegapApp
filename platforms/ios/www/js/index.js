...
function discoverArtist() {
	...
	lastfm.artist.getEvents({artist: searchedArtist, limit: 5, autocorrect: "1"}, {success: function(data){
		...
		for(var i = 0; i < data.events.event.length; i++) {
			...
			document.getElementById("artistResultEventList").innerHTML += '<li id="' + infoURL + '" class="table-view-cell media"> <a> <img class="media-object pull-left" src="' + eventImage + '"> <div class="media-body">' + data.events.event[i].venue.name + '<p>Date: ' + data.events.event[i].startDate + '</p> </div> </a>'
		}
	}, error: function(code, message){
  		console.log("Oh No an Error!");
	}});
